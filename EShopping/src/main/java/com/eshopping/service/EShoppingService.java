...
public class EShoppingService extends BaseService {
	...
	public void addShoppingItem(AbsShoppingItem item) {
		...
		if (item.getType() == AbsShoppingItem.TYPE_SINGLE) {
			...
			esitem.setBannerUrl(((SingleShoppingItem)item).getBannerUrl());
			esitem.setShopShowUrl(((SingleShoppingItem)item).getShopShowUrl());
	...
	public List<AbsShoppingItem> queryItemList(long cateId, int start, int fetchCount) {
		...
		String sql = " select eci.es_category_id, eci.es_item_id, eci.item_type, esi.name as iname, esi.price as iprice, esi.BANNER_PATH as iBANNER_PATH, esci.name as ciname, esci.price as ciprice, esci.pic_path as cipic_path, "+
					 "  esi.LIST_PATH as iLIST_PATH " +
			...
			if (size > 0) {
				...
				for (int i = 0; i < size; i++) {
					...
					if (type == AbsShoppingItem.TYPE_SINGLE) {
						...
						ss.setBannerUrl((String)key[5]);
						ss.setShopShowUrl((String)key[8]);
						queryList.add(ss);
					} else if (type == AbsShoppingItem.TYPE_COMBO) {
	...
	public List<CategoryItemSpecialType> getCategorySpecialType(Category cate) {
		...
		Session sess = openSession();
		Query query = sess.createQuery(" from ESCatgoryItemSpecialType st where st.categoryId =:ci order by group ");
	...
	public List<CategoryItemSpecialType> getCommonSpecialType() {
		List<CategoryItemSpecialType> list = null;
		Session sess = openSession();
		Query query = sess.createQuery(" from ESCatgoryItemSpecialType st where st.categoryId = 0  order by group asc");
		List<ESCatgoryItemSpecialType> queryList = query.list();
		if (queryList.size() > 0) {
			list = new ArrayList<CategoryItemSpecialType>(queryList.size());
			for (ESCatgoryItemSpecialType escit : queryList) {
	...
	public void addESCatgoryItemSpecialType(CategoryItemSpecialType  type) {
		...
		esType.setGroup(type.getGroup().getId());
	...
	public void addESCatgoryItemSpecialGroup(CategoryItemSpecialTypeGroup group) {
		if (group == null || group.getBelongs() == null) {
			return;
		}
		List<CategoryItemSpecialType> types = group.getTypes();
		...
		String hql = " select max(TYPE_GROUP) + 1   from ESCatgoryItemSpecialType ";
		...
		if (list == null || list.size() <= 0) {
			...
			if (value == null) {
				...
				groupId = (Integer)value;
			}
		}
		
		group.setId(groupId);
		Transaction t = beginTransaction(session);
		for (CategoryItemSpecialType type : types) {
			ESCatgoryItemSpecialType esType = new ESCatgoryItemSpecialType();
			esType.setCategoryId(type.getCategory().getId());
			esType.setGroup(type.getGroup().getId());
			esType.setGroupName(type.getGroupName());
	...
	public void deleteESCatgoryItemSpecialGroup(CategoryItemSpecialTypeGroup group) {
		if (group == null) {
			return;
		}
		...
		Session session = openSession();
		//Get next group number;
		String hql = " delete  ESCatgoryItemSpecialType  where group =:gid";
		Transaction t = beginTransaction(session);
		Query query = session.createQuery(hql);
		query.setInteger("gid", group.getId());
		query.executeUpdate();
		t.commit();
		session.close();
